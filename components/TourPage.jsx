...
const TourPage = () => {
  ...
  const filteredTours = tourData.filter((tour) => {
    // Split the locationFilter into individual words
    const searchWords = locationFilter.toLowerCase().split(" ");

    // Check if ANY of the search words are present in the tour location
    const matchesLocation =
      searchWords.length === 0 || // If no search term, include all tours
      searchWords.some((word) => tour.location.toLowerCase().includes(word));

    // Check if the date matches (if dateFilter is provided)
